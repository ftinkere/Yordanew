...
<template>
  <Layout>
    <template v-if="language.authorId === userId" #top-right>
    ...
    <div class="flex flex-col gap-2 items-center w-full">
      ...
      <div class="flex flex-row items-center justify-end gap-4 h-fit w-full">
        ...
        <Link :href="`/profile/${language.authorId}`" class="cursor-pointer">
          <div class="flex flex-row gap-2 items-center h-8 ps-4 rounded-full bg-black/20 hover:bg-black/40">
            Author
            <UAvatar :src="`/uploaded-files/avatars/${language.authorId}`" />
          </div>
        </Link>
      </div>
      
      <div class="ProseMirror self-start" v-html="language.description.content" />
    </div>
  </Layout>
</template>
